...
VESC::VESC(uint32_t can_id, double gear_ratio)
{
  logger_ = std::make_shared<rclcpp::Logger>(
    rclcpp::get_logger("controller_manager.resource_manager.hardware_component.system.DiffBot"));
  setup(can_id, gear_ratio);
}

void VESC::setup(uint32_t can_id, double gear_ratio)
{
  can_id_ = can_id;
  gear_ratio_ = gear_ratio;
}

int32_t VESC::vel_to_rpm(double vel)
{
  // rads/s to rpm
  return static_cast<int32_t>((vel * SECONDS_PER_MINUTE) / FULL_ROTATION_RADS);
}

double VESC::rpm_to_vel(int32_t rpm)
{
  return static_cast<double>((rpm * FULL_ROTATION_RADS) / SECONDS_PER_MINUTE);
}

can_frame VESC::create_rpm_cmd(int32_t rpm)
{
  can_frame rpm_msg{};
  rpm_msg.can_id = can_id_ | RPM_CMD_ID | CAN_EFF_FLAG;
  rpm_msg.can_dlc = 4;
  int32_t erpm = rpm_to_vel(rpm) * gear_ratio_ * 2; //the 2 has something to do with motor poles/erpm
...
{
  RCLCPP_INFO(rclcpp::get_logger("test"), "dlc: '%d'", static_cast<int>(frame.can_dlc));
  if(static_cast<int>(frame.can_dlc) < 4)
  {
    RCLCPP_INFO(rclcpp::get_logger("test"), "Leaving callback");
    return;
  }
  RCLCPP_INFO(rclcpp::get_logger("test"), "Inside callback");
  ...
  RCLCPP_INFO(rclcpp::get_logger("test"), "erpm: '%d'", ERPM);
  int32_t RPM = ERPM / (gear_ratio_ * 2); 
